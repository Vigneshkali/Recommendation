...
def getTrackFeatures(id):
    ...
    director = track_info['director']['name']
    hero = track_info['director']['hero'][0]['name']
    release_date = track_info['director']['release_date']
    length = track_info['duration_ms']
    popularity = track_info['popularity']

    track_data = [name, director, hero] #, release_date, length, popularity
    return track_data
...
for i in range(len(track_ids)):
    ...
    df = pd.DataFrame(track_list, columns = ['Name','director','hero']) # ,'Release_date','Length','Popularity'
